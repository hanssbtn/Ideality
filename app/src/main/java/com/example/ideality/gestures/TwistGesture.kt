...
import com.example.ideality.toDegrees
...
class TwistGesture(
    ...
    companion object {
        ...
        private fun calculateDeltaRotation(
            currentPosition1: Float3,
            currentPosition2: Float3,
            previousPosition1: Float3,
            previousPosition2: Float3
        ): Float {
            val currentDirection: Float3 = currentPosition1.minus(currentPosition2).normalize()
            val previousDirection: Float3 = previousPosition1.minus(previousPosition2).normalize()

            return (acos(dot(currentDirection, previousDirection)).toDegrees() * sign(previousDirection.x * currentDirection.y - previousDirection.y * currentDirection.x))
        }
    }
    ...
    override fun canStart(motionEvent: MotionEvent): Boolean {
        ...
        if (deltaPosition1.equals(Float3())
            || deltaPosition2.equals(Float3())
        ) {
            return false
        }
