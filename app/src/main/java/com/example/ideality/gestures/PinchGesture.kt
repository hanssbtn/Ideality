...
import com.example.ideality.length
import com.example.ideality.normalize
...
class PinchGesture(
    ...
    private var startPosition1: Float3 =
        GesturePointersUtility.motionEventToPosition(motionEvent, pointerId1)

    private var startPosition2: Float3 =
        GesturePointersUtility.motionEventToPosition(motionEvent, pointerId2)

    private var previousPosition1: Float3 = startPosition1
    private var previousPosition2: Float3 = startPosition2
    ...
    override fun canStart(motionEvent: MotionEvent): Boolean {
        ...
        val firstToSecond: Float3 = startPosition1.minus(startPosition2)
        val firstToSecondDirection: Float3 = firstToSecond.normalize()
        val newPosition1: Float3 = GesturePointersUtility.motionEventToPosition(motionEvent, pointerId1)
        val newPosition2: Float3 = GesturePointersUtility.motionEventToPosition(motionEvent, pointerId2)
        val deltaPosition1: Float3 = newPosition1.minus(previousPosition1)
        val deltaPosition2: Float3 = newPosition2.minus(previousPosition2)
        previousPosition1 = newPosition1
        previousPosition2 = newPosition2
        val dot1: Float = dot(deltaPosition1.normalize(), firstToSecondDirection.unaryMinus())
        val dot2: Float = dot(deltaPosition2.normalize(), firstToSecondDirection)
        ...
        if (deltaPosition1.equals(Float3()).not() && abs(dot1) < dotThreshold) {
            return false
        }

        // Check angle of motion for the second touch.
        if (deltaPosition2.equals(Float3()).not() && abs(dot2) < dotThreshold) {
            return false
        }

        val startGap: Float = firstToSecond.length()
        gap = newPosition1.minus(newPosition2).length()
    ...
    override fun updateGesture(motionEvent: MotionEvent): Boolean {
        ...
        val newPosition1: Float3 = GesturePointersUtility.motionEventToPosition(motionEvent, pointerId1)
        val newPosition2: Float3 = GesturePointersUtility.motionEventToPosition(motionEvent, pointerId2)
        val newGap: Float = newPosition1.minus(newPosition2).length()
